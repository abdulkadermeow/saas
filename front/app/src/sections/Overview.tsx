...
export default function Overview({ onNavigate }: { onNavigate: (p: PageKey) => void }) {
  ...
  const monthMessages = WEEK_STATS.reduce((a, b) => a + b.messages, 0)
  ...
  const stats = [
    ...
    { label: 'رصيد الرسائل المتبقي', value: messageBalance.toLocaleString('ar'), icon: Wallet, hint: `من ${messageTotal.toLocaleString('ar')}` },
  ]
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white p-5">
          ...
          <div className="flex items-center gap-2">
            ...
            <Switch dir="ltr" checked={rafiq.active} onCheckedChange={(v) => updateRafiq({ active: v })} />
          </div>
        </div>
        ...
        <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white p-5">
          ...
          <div className="flex items-center gap-2">
            ...
            <Switch dir="ltr" checked={rafiqa.active} onCheckedChange={(v) => updateRafiqa({ active: v })} />
          </div>
        </div>
      </div>
            ...
            {conversations.slice(0, 5).map((c) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className="truncate text-sm font-bold text-slate-800">{c.customerName}</span>
                ...
                <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold ${CATEGORY_COLORS[c.category]}`}>
                  {c.category}
                </span>
              </button>
            ))}
